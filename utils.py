...
def get_ln_remove_chars(typed_keys: str, program_name:str) -> int:
    original_clipboard = pyperclip.paste()
    pynput_key_press(Key.end)
    time.sleep(0.1)
    pynput_shortcut(Key.shift, Key.home)
    time.sleep(0.1)
    if "конфигуратор" in program_name:
            pynput_shortcut(Key.alt_l, Key.shift)    
    time.sleep(0.1)
...
def insert_snippet(typed_keys, listbox, snippets, popup, program_name, comment):
    ...
    if index:
        ...
        remove_num = get_ln_remove_chars(typed_keys, program_name)
        ...
        original_clipboard = pyperclip.paste()
        clean_snippet_text = ''
        if comment == '' or snippet_text.find("'") == -1:
